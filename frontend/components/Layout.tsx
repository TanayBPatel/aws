...
import { Home, BarChart2, PieChart, Settings, Calendar, Users, LogOut } from 'lucide-react';
      ...
      <div className="hidden md:flex fixed left-0 top-0 h-full w-20 bg-cardDark border-r border-borderBase flex-col items-center py-8 z-50 transition-colors duration-300">
        ...
        <div className="flex flex-col space-y-8">
          {navItems.map((item) => (
            <button
              ...
              className={`p-3 rounded-xl transition-all duration-300 ${location.pathname === item.path
        ...
        <div className="mt-auto mb-8">
          <button
            ...
            className="p-3 rounded-xl text-textSecondary hover:text-error hover:bg-surfaceLight transition-all duration-300"
            title="Logout"
          >
            <LogOut size={22} />
          </button>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.path}
                onClick={() => navigate(item.path)}
                className={`flex items-center justify-center transition-all duration-300 shrink-0 ${isActive
