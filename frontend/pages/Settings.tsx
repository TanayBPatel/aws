    ...
    <div className="space-y-2 animate-in slide-in-from-right duration-200">
      {['English (US)', 'English (IN)', 'Hindi', 'Spanish', 'French'].map(lang => (
        <button
          ...
          className={`w-full p-4 rounded-xl border flex justify-between items-center transition-all ${formData.language === lang
        ...
        activeAlerts.map(alert => {
          ...
          return (
            <div key={alert.id} className="bg-cardDark p-4 rounded-2xl border border-borderBase flex justify-between items-center">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${alert.type === 'above' ? 'bg-accentLime/10 text-accentLime' : 'bg-error/10 text-error'
                  }`}>
                  {alert.type === 'above' ? '↑' : '↓'}
                </div>
                <div>
