      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-6">
        {['All', 'Orders', 'Deposits', 'Withdrawals'].map((filter) => (
            <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200 border border-transparent ${
                    activeTab === filter
                  ...
                  <div key={tx.id} className="bg-cardDark p-4 rounded-2xl border border-borderBase flex items-center justify-between shadow-sm hover:bg-surfaceLight transition-colors">
                      ...
                      <div className="text-right">
                          <p className={`font-medium ${
                              tx.type === 'buy' || tx.type === 'withdraw' ? 'text-textPrimary' : 'text-accentLime'
                          }`}>
                              {tx.type === 'buy' || tx.type === 'withdraw' ? '-' : '+'}
                              ₹{tx.amount?.toLocaleString('en-IN') || '0'}
                          </p>
                          <p className={`text-xs capitalize ${
                              tx.status === 'completed' ? 'text-accentLime' : tx.status === 'failed' ? 'text-error' : 'text-textSecondary'
                          }`}>{tx.status || 'pending'}</p>
                      </div>
                  </div>
              ))
